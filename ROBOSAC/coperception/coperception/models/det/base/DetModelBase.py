...
import numpy as np
from coperception.utils.bandwidth_logger import get_bandwidth_logger
...
class DetModelBase(nn.Module):
    ...
    def __init__(
        ...
        self._bandwidth_logger = get_bandwidth_logger()
    ...
    def build_neighbors_feature_list(
        ...
        for j in range(num_agent):
            if j != agent_idx:
                ...
                if pert is not None and attacker_list is not None and j in attacker_list:
                    # clip
                ...
                # Bandwidth measurement hook - Enhanced for V2X-Sim
                if hasattr(self, '_bandwidth_logger') and self._bandwidth_logger is not None:
                    # Calculate actual bytes transmitted for this feature map
                    bytes_transmitted = warp_feat.numel() * warp_feat.element_size()
                    
                    # Log detailed transmission information
                    self._bandwidth_logger.log_transmission(
                        sender_agent=j,
                        receiver_agent=agent_idx,
                        bytes_transmitted=bytes_transmitted,
                        feature_shape=warp_feat.shape,
                        tensor_dtype=str(warp_feat.dtype),
                        compression_ratio=getattr(self, '_compression_ratio', 1.0)
                    )
