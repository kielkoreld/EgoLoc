...
def get_jaccard_index(config, num_agent_list, padded_voxel_point, reg_target, anchors_map, gt_max_iou, result_1, result_2, ego_idx=None):
    ...
    # ego_idx가 전달되지 않으면 환경 변수에서 가져오기
    if ego_idx is None:
        env_ego_idx = os.environ.get("EGOL0C_CURRENT_EGO_IDX", "")
        if env_ego_idx:
            try:
                ego_idx = int(env_ego_idx)
            except Exception:
                # 환경 변수 파싱 실패 시 args에서 가져오기 (하위 호환성)
                try:
                    import sys
                    module = sys.modules.get(__name__)
                    if module and hasattr(module, 'args') and hasattr(module.args, 'ego_agent'):
                        ego_idx = module.args.ego_agent
                    else:
                        raise ValueError("ego_idx must be provided when args is not available")
                except (AttributeError, ValueError):
                    raise ValueError("ego_idx must be provided when args is not available")
        else:
            # 환경 변수도 없으면 모듈 레벨 args에서 가져오기 (하위 호환성)
            try:
                import sys
                module = sys.modules.get(__name__)
                if module and hasattr(module, 'args') and hasattr(module.args, 'ego_agent'):
...
# ==== EgoLoc minimal detection exporter (guarded by env vars) ====
def _egoloc_get_export_plan():
    export_dir = os.environ.get("EGOL0C_DET_EXPORT_DIR", "").strip()
    frames_str = os.environ.get("EGOL0C_DET_EXPORT_FRAMES", "").strip()
    if not export_dir:
        return None, set()
    frames = set()
    if frames_str:
        for tok in frames_str.split(","):
            try:
                ...
                pass
    return export_dir, frames

def _egoloc_convert_det_array_to_xyxy_list(det_arr):
    # det_arr shape: (N, 8 or 9). First 8 are 4-corner (x1,y1,x2,y2,x3,y3,x4,y4)
    if det_arr is None or len(det_arr) == 0:
        return []
    boxes = []
    import numpy as _np
    arr = _np.array(det_arr)
    cols = arr.shape[1]
    for row in arr:
        if cols >= 8:
            xs = [row[0], row[2], row[4], row[6]]
            ys = [row[1], row[3], row[5], row[7]]
            ...
            continue
        conf = float(row[-1]) if cols >= 9 else 1.0
        boxes.append({
            "x1": x1, "y1": y1, "x2": x2, "y2": y2,
            "confidence": conf, "class": "car", "source": "robosac"
        })
    return boxes

def _egoloc_export_current_detection_if_needed(*, det_results_local, ego_idx, scene_id, frame_idx):
    """
    EgoLoc 검증 단계에서만 detection 결과를 JSON으로 export
    
    검증 단계인지 확인하기 위해 EGOL0C_VALIDATION_MODE 환경 변수를 확인합니다.
    이 환경 변수가 설정되어 있지 않으면 (baseline 실행 등) export를 수행하지 않습니다.
    """
    # 검증 모드 플래그 확인: 검증 단계가 아닌 경우 (baseline 등) export 안 함
    validation_mode = os.environ.get("EGOL0C_VALIDATION_MODE", "").strip()
    if not validation_mode or validation_mode.lower() not in ("1", "true", "yes"):
        return  # 검증 단계가 아니면 export 안 함
    
    export_dir, frames = _egoloc_get_export_plan()
    if not export_dir:
        return
    # If frames filter provided, export only when matched
    if frames and int(frame_idx) not in frames:
        return
    try:
        # 파일명은 agent ID 기반으로 생성 (agent{id}.json)
        # 환경 변수에서 agent ID 가져오기 (이미 ego_idx 파라미터로 전달되지만, 환경 변수와 일치 확인)
        env_ego_idx = os.environ.get("EGOL0C_CURRENT_EGO_IDX", "")
        if env_ego_idx:
            try:
                env_agent_id = int(env_ego_idx)
                # 환경 변수와 파라미터 일치 확인
                if env_agent_id != ego_idx:
                    print(f"[egoloc] WARNING: env EGOL0C_CURRENT_EGO_IDX={env_agent_id} != param ego_idx={ego_idx}, using param={ego_idx}")
                file_agent_id = ego_idx  # 파라미터 우선 사용
            except Exception:
                file_agent_id = ego_idx
        else:
            file_agent_id = ego_idx
        
        # 실제 agent ID로 detection 결과 가져오기
        det_list_nested = det_results_local[ego_idx]
        if not det_list_nested:
            return
        det_arr = det_list_nested[0][0]
        boxes = _egoloc_convert_det_array_to_xyxy_list(det_arr)
        if not boxes:
            return
        
        # 프레임별 저장: scene_{id}/frame_{fid}/agent{id}.json (agent ID 기반 파일명)
        base_dir = export_dir
        frame_dir = os.path.join(base_dir, f"frame_{int(frame_idx)}")
        os.makedirs(frame_dir, exist_ok=True)
        out_path = os.path.join(frame_dir, f"agent{file_agent_id}.json")
        with open(out_path, "w", encoding="utf-8") as f:
            json.dump(boxes, f, ensure_ascii=False)
        print(f"[egoloc] exported detections: {out_path} ({len(boxes)} boxes) - agent_id={file_agent_id}")
    except Exception as e:
        print(f"[egoloc] export failed: {e}")
        import traceback
        traceback.print_exc()
...
def cal_robosac_steps(num_agent, num_consensus, num_attackers):
    ...
    N = np.ceil(np.log(1 - 0.99) / np.log(1 - np.power(1 - eta, num_consensus))).astype(int) #divide by 0 encountered in log
    return N
...
def main(args):
    ...
    if args.log:
        log_file_name = os.path.join(model_save_path, "{}attackers_{}_{}.txt".format(args.number_of_attackers, args.robosac, time_str()))
    ...
    # 검증 모드에서 각 검증마다 상태 초기화: optimizer 상태 정리
    # 특히 연속 실행 시 이전 검증의 optimizer 상태가 남아있을 수 있으므로 명시적 초기화
    validation_mode = os.environ.get("EGOL0C_VALIDATION_MODE", "").strip()
    if validation_mode and validation_mode.lower() in ("1", "true", "yes"):
        # 검증 모드: optimizer 상태 완전 초기화
        try:
            # optimizer의 모든 파라미터 그룹 상태 초기화
            for param_group in fafmodule.optimizer.param_groups:
                # param_group 자체는 초기화할 수 없지만, 파라미터들의 gradient는 초기화 가능
                pass
            # optimizer state_dict 초기화는 복잡하므로, 대신 각 파라미터의 gradient만 명시적으로 관리
            # 실제 optimizer state는 backward() 호출 시 자동으로 관리됨
        except Exception:
            pass
    ...
    for cnt, sample in enumerate(tqdm(validation_data_loader)):
        ...
        if hasattr(args, 'ego_agent'):
            if int(idx) == 0:  # 첫 프레임에서만 출력
                print(f"[robosac] main 함수 내부: seq_name={seq_name}, frame_idx={idx}, args.ego_agent={args.ego_agent}")
        ...
        # 검증용 프레임 제한 (EgoLoc 검증 시 3프레임만 처리)
        if hasattr(args, 'max_frames_for_validation') and args.max_frames_for_validation is not None:
            if int(idx) >= args.max_frames_for_validation:
                continue
        ...
        if args.robosac == 'upperbound':
            ...
            try:
                # 디버깅: args.ego_agent 확인
                export_ego_idx = args.ego_agent
                print(f"[robosac] upperbound export: frame={idx}, args.ego_agent={export_ego_idx}")
                _egoloc_export_current_detection_if_needed(det_results_local=det_results_local, ego_idx=export_ego_idx, scene_id=int(seq_name), frame_idx=int(idx))
            except Exception:
                pass
            continue
            ...
            try:
                # 디버깅: args.ego_agent 확인
                export_ego_idx = args.ego_agent
                print(f"[robosac] lowerbound export: frame={idx}, args.ego_agent={export_ego_idx}")
                _egoloc_export_current_detection_if_needed(det_results_local=det_results_local, ego_idx=export_ego_idx, scene_id=int(seq_name), frame_idx=int(idx))
            except Exception:
                pass
            continue
            ...
            # EgoLoc 공격 시나리오 확인 및 처리
            egoloc_attack_active = False
            try:
                # import 시도 (여러 방법 시도)
                handle_egoloc_attack_scenario = None
                try:
                    # 방법 1: 상대 import
                    from .egoloc_attack_handler import handle_egoloc_attack_scenario
                except ImportError:
                    try:
                        # 방법 2: 절대 import
                        from coperception.tools.det.egoloc_attack_handler import handle_egoloc_attack_scenario
                    except ImportError:
                        try:
                            # 방법 3: sys.path를 이용한 import
                            # os와 sys는 이미 파일 상단에서 import되어 있음
                            current_dir = os.path.dirname(os.path.abspath(__file__))
                            if current_dir not in sys.path:
                                sys.path.insert(0, current_dir)
                            import egoloc_attack_handler
                            handle_egoloc_attack_scenario = egoloc_attack_handler.handle_egoloc_attack_scenario
                        except ImportError as e:
                            print_and_write_log(f"[EgoLoc Debug] 모든 import 방법 실패: {e}")
                            raise
                
                if handle_egoloc_attack_scenario is None:
                    raise ImportError("handle_egoloc_attack_scenario를 import할 수 없습니다")
                
                # EgoLoc 공격 시나리오인지 확인
                has_egoloc_attack_mode = hasattr(args, 'egoloc_attack_mode')
                # --attack 플래그가 활성화된 경우 Baseline과 EgoLoc 모두 egoloc_attack_handler 사용
                if has_egoloc_attack_mode and args.egoloc_attack_mode:
                    # 먼저 기존 ROBOSAC 로직으로 attacker_list 생성 (공격 최적화는 하지 않음)
                    num_sensor = num_agent_list[0][0]
                    ego_idx = args.ego_agent
                    all_agent_list = [i for i in range(num_sensor)]
                    
                    # Baseline 실행 시 공격자 리스트 확인 (EGOL0C_BASELINE_ATTACKER_LIST 사용)
                    ...
                    # 기존 ROBOSAC의 attacker_list 생성
                    # --attack이 활성화된 경우, 기존 ROBOSAC 공격자는 환경 변수가 아닌 랜덤 선택으로 생성
                    # (환경 변수는 --attack으로 추가된 공격자를 위한 것이므로)
                    robosac_attacker_list = None
                    
                    # We always trust ourself
                    all_agent_list_wo_ego = all_agent_list.copy()
                    all_agent_list_wo_ego.remove(ego_idx)
                    ...
                    if args.robosac == 'fix_attackers':
                        ...
                        if fix_attackers_generated == False:
                            robosac_attacker_list = random.sample(all_agent_list_wo_ego, k=args.number_of_attackers)
                            fix_attackers_generated = True
                    else:
                        robosac_attacker_list = random.sample(all_agent_list_wo_ego, k=args.number_of_attackers)
                    
                    print_and_write_log(f"[ROBOSAC Attack] 기존 ROBOSAC 공격자 리스트 (랜덤 선택): {robosac_attacker_list}")
                    
                    egoloc_attack_active = True
                    pert, attacker_list_egoloc = handle_egoloc_attack_scenario(
                        args, data, num_sensor, ego_idx, all_agent_list, device,
                        fafmodule, batch_size, pseudo_gt, print_and_write_log,
                        robosac_attacker_list=robosac_attacker_list  # 기존 ROBOSAC의 attacker_list 전달
                    )
                    if pert is not None:
                        # EgoLoc 공격 시나리오가 활성화된 경우
                        attacker_list = attacker_list_egoloc
                        # EgoLoc 공격 시나리오: data에 모든 필요한 값 명시적 설정
                        # (egoloc_attack_handler에서 이미 설정했지만, 명시적으로 다시 설정하여 확실히 함)
                        data['attacker_list'] = attacker_list
                        data['eps'] = args.eps
                        data['no_fuse'] = False
                        data['pert'] = pert.to(device)  # requires_grad=False로 detach된 상태
                        data['ego_agent'] = args.ego_agent  # ego_agent도 명시적으로 설정
                        # 공격자 리스트는 egoloc_attack_handler에서 이미 출력됨
                    else:
                        egoloc_attack_active = False
                else:
                    egoloc_attack_active = False
            except ImportError:
                # EgoLoc 모듈이 없는 경우 기존 로직 사용
                egoloc_attack_active = False
            except Exception:
                egoloc_attack_active = False
            
            # 기존 ROBOSAC 로직 (EgoLoc 공격 시나리오가 아닌 경우)
            if not egoloc_attack_active:
                ...
                if is_baseline and baseline_attacker_list_env:
                    # Baseline 실행: 환경 변수에서 공격자 리스트 읽기
                    try:
                        attacker_list = [int(x.strip()) for x in baseline_attacker_list_env.split(",") if x.strip()]
                        print_and_write_log(f"[Baseline Attack] Baseline 공격자 리스트 (환경 변수): {attacker_list}")
                    except Exception as e:
                        print_and_write_log(f"[Baseline Attack] Baseline 환경 변수 파싱 실패: {e}, 랜덤 공격자 선택")
                        attacker_list = None
                else:
                    attacker_list = None
                
                # 공격자 리스트가 설정되지 않은 경우 기존 로직 사용 (랜덤 선택)
                if attacker_list is None:
                    ...
                    if args.robosac == 'fix_attackers':
                        # Generate random attackers and keep them always attacking in the scene
                        if fix_attackers_generated == False:
                            attacker_list = random.sample(all_agent_list_wo_ego, k=args.number_of_attackers)
                            fix_attackers_generated = True
                    else:
                        attacker_list = random.sample(all_agent_list_wo_ego, k=args.number_of_attackers)
            ...
            if args.robosac == 'no_defense':
                ...
                try:
                    # 디버깅: args.ego_agent 확인
                    export_ego_idx = args.ego_agent
                    print(f"[robosac] no_defense export: frame={idx}, args.ego_agent={export_ego_idx}")
                    _egoloc_export_current_detection_if_needed(det_results_local=det_results_local, ego_idx=export_ego_idx, scene_id=int(seq_name), frame_idx=int(idx))
                except Exception:
                    pass
                continue
            ...
            if args.robosac == 'fix_attackers':
                ...
                if fix_attackers_collab_agent_list == []:
                    ...
                    for step in range(1, args.step_budget + 1):
                        ...
                        jac_index = get_jaccard_index(
                            config, num_agent_list, padded_voxel_points, reg_target, anchors_map, gt_max_iou, result_reference, result, ego_idx=args.ego_agent)
                        print_and_write_log(
                ...
                try:
                    # 디버깅: args.ego_agent 확인
                    export_ego_idx = args.ego_agent
                    print(f"[robosac] robosac_validation export: frame={idx}, args.ego_agent={export_ego_idx}")
                    _egoloc_export_current_detection_if_needed(det_results_local=det_results_local, ego_idx=export_ego_idx, scene_id=int(seq_name), frame_idx=int(idx))
                except Exception:
                    pass
            ...
            if args.robosac == "probing" :
                ...
                all_agent_list = [i for i in range(num_sensor)]
                # We always trust ourself
                all_agent_list_original = all_agent_list.copy()  # 원본 보관 (공격자 제외 로직에서 사용)
                all_agent_list.remove(ego_idx)
                # Not including ego agent, since ego agent is always used.
                collab_agent_list = []
                ...
                # EgoLoc 공격 시나리오에서 추가된 공격자(검증 전 1순위)만 collab_agent_list에서 배제
                # Baseline과 EgoLoc 구분: EGOL0C_ATTACKER_LIST 환경 변수 존재 여부로 판단
                # Baseline에서는 공격자가 자아 차량(ego_agent)이므로 collab_agent_list에 포함되지 않음
                # EgoLoc에서는 추가된 공격자(검증 전 1순위)만 배제하고, 기존 ROBOSAC 공격자는 배제하지 않음
                # (기존 ROBOSAC 공격자는 ROBOSAC 자체의 합의를 통해 제거 가능)
                egoloc_attacker_list_env = os.environ.get("EGOL0C_ATTACKER_LIST", "").strip()
                baseline_attacker_list_env = os.environ.get("EGOL0C_BASELINE_ATTACKER_LIST", "").strip()
                is_egoloc = bool(egoloc_attacker_list_env)
                
                # EgoLoc 실행 시 추가된 공격자(검증 전 1순위)만 저장 (각 step에서 사용)
                attacker_list_for_exclusion = None
                if is_egoloc and egoloc_attacker_list_env:
                    # 환경 변수에서 검증 전 1순위 공격자 리스트 파싱
                    try:
                        attacker_list_for_exclusion = [int(x.strip()) for x in egoloc_attacker_list_env.split(",") if x.strip()]
                        print_and_write_log(f"[EgoLoc] 추가된 공격자(검증 전 1순위) {attacker_list_for_exclusion}를 collab_agent_list에서 배제합니다. (기존 ROBOSAC 공격자는 배제하지 않음)")
                    except Exception as e:
                        print_and_write_log(f"[EgoLoc] 추가된 공격자 리스트 파싱 실패: {e}, 배제하지 않음")
                        attacker_list_for_exclusion = None
                ...
                for step in range(1, args.step_budget + 1):
                    ...
                    # EgoLoc 실행 시 collab_agent_list에서 추가된 공격자(검증 전 1순위)만 제거 및 대체 agent 추가
                    if is_egoloc and attacker_list_for_exclusion is not None:
                        original_size = len(collab_agent_list)
                        # collab_agent_list에서 추가된 공격자(검증 전 1순위)만 제거
                        attackers_in_collab = [attacker for attacker in attacker_list_for_exclusion if attacker in collab_agent_list]
                        for attacker in attackers_in_collab:
                            collab_agent_list.remove(attacker)
                        
                        # 제거된 공격자 수만큼 새로운 agent 추가
                        num_removed = len(attackers_in_collab)
                        if num_removed > 0:
                            # collab_agent_list에 포함되지 않고, 추가된 공격자 리스트에도 포함되지 않은 agent 중에서 샘플링
                            # (추가된 공격자를 다시 추가하지 않도록, 기존 ROBOSAC 공격자는 포함 가능)
                            # all_agent_list_original을 사용하여 ego_agent를 포함한 전체 리스트에서 샘플링
                            # 단, ego_agent는 항상 사용되므로 제외
                            available_agents = [agent for agent in all_agent_list_original 
                                              if agent != ego_idx
                                              and agent not in collab_agent_list 
                                              and agent not in attacker_list_for_exclusion]
                            if len(available_agents) >= num_removed:
                                # 충분한 agent가 있으면 추가
                                replacement_agents = random.sample(available_agents, k=num_removed)
                                collab_agent_list.extend(replacement_agents)
                                print_and_write_log(f"[EgoLoc] Step {step}: 추가된 공격자(검증 전 1순위) {attackers_in_collab}를 제거하고 {replacement_agents}를 추가했습니다.")
                            else:
                                # 추가할 agent가 부족한 경우
                                if len(available_agents) > 0:
                                    # 가능한 만큼만 추가
                                    collab_agent_list.extend(available_agents)
                                    print_and_write_log(f"[EgoLoc Warning] Step {step}: 추가된 공격자(검증 전 1순위) {attackers_in_collab}를 제거했지만, 추가할 agent가 부족합니다. (제거: {num_removed}, 추가 가능: {len(available_agents)}, 실제 추가: {len(available_agents)})")
                                else:
                                    print_and_write_log(f"[EgoLoc Warning] Step {step}: 추가된 공격자(검증 전 1순위) {attackers_in_collab}를 제거했지만, 추가할 agent가 없습니다. collab_agent_list 크기가 {original_size}에서 {len(collab_agent_list)}로 감소했습니다.")
                                
                                # collab_agent_list 크기가 원래 크기보다 작아진 경우 조정
                                if len(collab_agent_list) < original_size:
                                    size_diff = original_size - len(collab_agent_list)
                                    if args.robosac_k == None:
                                        # consensus_set_size 조정
                                        if consensus_set_size >= size_diff:
                                            consensus_set_size = len(collab_agent_list)
                                            print_and_write_log(f"[EgoLoc Warning] Step {step}: consensus_set_size를 {consensus_set_size}로 조정했습니다.")
                                        else:
                                            consensus_set_size = max(0, consensus_set_size - size_diff)
                                            print_and_write_log(f"[EgoLoc Warning] Step {step}: consensus_set_size를 {consensus_set_size}로 조정했습니다.")
                                    else:
                                        # args.robosac_k 조정
                                        if args.robosac_k >= size_diff:
                                            args.robosac_k = len(collab_agent_list)
                                            print_and_write_log(f"[EgoLoc Warning] Step {step}: args.robosac_k를 {args.robosac_k}로 조정했습니다.")
                                        else:
                                            args.robosac_k = max(0, args.robosac_k - size_diff)
                                            print_and_write_log(f"[EgoLoc Warning] Step {step}: args.robosac_k를 {args.robosac_k}로 조정했습니다.")
                                    
                                    # consensus_set_size가 0이 된 경우
                                    if args.robosac_k == None and consensus_set_size == 0:
                                        print_and_write_log(f"[EgoLoc Warning] Step {step}: consensus_set_size가 0이 되었습니다. 이 step은 건너뜁니다.")
                                        continue
                                    elif args.robosac_k is not None and args.robosac_k == 0:
                                        print_and_write_log(f"[EgoLoc Warning] Step {step}: args.robosac_k가 0이 되었습니다. 이 step은 건너뜁니다.")
                                        continue
                    data['collab_agent_list'] = collab_agent_list
                    data['no_fuse'] = False
                    data['pert'] = pert.to(device)
                    ...
                    # We use jaccard index to define the difference between two bbox sets
                    jac_index = get_jaccard_index(config, num_agent_list, padded_voxel_points, reg_target, anchors_map, gt_max_iou, result_reference, result, ego_idx=args.ego_agent)
                ...
                try:
                    export_ego_idx = args.ego_agent
                    print(f"[robosac] robosac_mAP export: frame={idx}, args.ego_agent={export_ego_idx}")
                    _egoloc_export_current_detection_if_needed(det_results_local=det_results_local, ego_idx=export_ego_idx, scene_id=int(seq_name), frame_idx=int(idx))
                except Exception:
                    pass
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d",
        "--data",
        default="../../coperception/datasets/V2X-Sim-det/test",
        type=str,
